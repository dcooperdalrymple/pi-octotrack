...
void AlsaCallback(snd_async_handler_t *pcmCallback) {
    ...
    CallbackData *callbackData = (CallbackData*)snd_async_handler_get_callback_private(pcmCallback);

    available = snd_pcm_avail_update(handle);
    while (available >= callbackData->periodSize) {
        snd_pcm_mmap_writei(handle, callbackData->buffer, callbackData->periodSize);
        available = snd_pcm_avail_update(handle);
        if (available < 0) {
...
int main(int argc, char *argv[]) {
    ...
    snd_pcm_uframes_t bufferSize = periodSize * sizeof(sample_t);

    // Access
    error = snd_pcm_hw_params_set_access(handle, hwParams, SND_PCM_ACCESS_MMAP_INTERLEAVED);
    std::cout << "snd_pcm_hw_params_set_access: " << snd_strerror(error) << std::endl;

    // Format
    error = snd_pcm_hw_params_set_format(handle, hwParams, SND_PCM_FORMAT_S16_LE);
    std::cout << "snd_pcm_hw_params_set_format: " << snd_strerror(error) << std::endl;

    // Sample Rate
    error = snd_pcm_hw_params_set_rate_near(handle, hwParams, &sample_rate, 0);
    std::cout << "snd_pcm_hw_params_set_rate_near: " << snd_str_error(error) << " actual: " << sample_rate << std::endl;

    // Channels
    error = snd_pcm_hw_params_set_channels(handle, hwParams, &channels);
    std::cout << "snd_pcm_hw_params_set_channels: " << snd_str_error(error) << " actual: " << channels << std::endl;

    // Buffer Size
    error = snd_pcm_hw_params_set_buffer_size_near(handle, hwParams, &bufferSize);
    std::cout << "snd_pcm_hw_params_set_buffer_size_near: " << snd_strerror(error) << " actual: " << bufferSize << std::endl;

    // Period Size
    error = snd_pcm_hw_params_set_period_size_near(handle, hwParams, &periodSize, nullptr);
    std::cout << "snd_pcm_hw_params_set_period_size_near: " << snd_str_error(error) << " action: " << periodSize << std::endl;
    ...
    std::cout << "snd_pcm_sw_params_set_start_threshold: " << snd_strerror(error) << std::endl;
    error = snd_pcm_sw_params_set_avail_min(handle, swParams, periodSize);
    std::cout << "snd_pcm_sw_params_set_avail_min: " << snd_strerror(error) << std::endl;
    ...
    int16_t *buffer = (int16_t *)malloc(channels * periodSize * sizeof(int16_t));
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        if (error > 0) {
            std::cout << "snd_pcm_mmap_writei: " << error << " frames written" << std::endl;
        } else {
            std::cout << "snd_pcm_mmap_writei: " << snd_strerror(error) << std::endl;
        }
    }
